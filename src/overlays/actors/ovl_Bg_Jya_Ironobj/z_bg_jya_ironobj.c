...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* this, PlayState* play, EnIk* enIk) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        if (Rand_ZeroOne() < 0.26f) {
            Actor_SpawnFloorDustCircle(play, &pos, 200.0f, 1, D_808994E0[j & 7] * 4 + 60, D_808994E0[j & 7] * 4 + 80, 1);
        }
    }
}
...
void BgJyaIronobj_SpawnThroneParticles(BgJyaIronobj* this, PlayState* play, EnIk* enIk) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        if (Rand_ZeroOne() < 0.26f) {
            Actor_SpawnFloorDustCircle(play, &pos, 200.0f, 1, D_80899510[j & 7] * 4 + 60, D_80899510[j & 7] * 4 + 80, 1);
        }
    }
}
